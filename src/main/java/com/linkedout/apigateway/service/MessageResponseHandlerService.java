...
public class MessageResponseHandlerService {
   ...
   * 반환합니다.
   *
   * <p>Mono란? - 리액티브 프로그래밍에서 0 또는 1개의 결과를 비동기적으로 제공하는 발행자(Publisher)입니다. - "미래에 완료될 가능성이 있는 작업"을
   * 나타내며, 콜백 대신 선언적 방식으로 비동기 작업을 처리합니다. - 이를 통해 비동기 코드를 더 선형적이고 읽기 쉽게 작성할 수 있습니다.
   *
   ...
   *
   * <p>이 메서드는 RabbitMQ의 'gateway-response-queue'라는 큐에서 메시지를 수신하고, 해당 메시지의 correlationId를 사용하여 올바른
   * Sink를 찾아 응답을 전달합니다.
   *
   * <p>{@code @RabbitListener}: - 지정된 RabbitMQ 큐에서 메시지를 수신하는 메서드임을 나타내는 애노테이션 - Spring AMQP가 자동으로 이
   * 메서드를 메시지 리스너로 등록함 - 큐에 메시지가 도착하면 자동으로 이 메서드가 호출됨
   *
   * <p>{@code @ConditionalOnProperty}: - 지정된 속성 값에 따라 이 메서드를 메시지 리스너로 등록할지 여부를 결정하는 애노테이션 -
   * spring.rabbitmq.enabled 속성이 true일 때만 이 메서드가 활성화됨 - matchIfMissing=true로 설정되어 있어 해당 속성이 없으면 기본값은
   * true로 간주됨
   *
