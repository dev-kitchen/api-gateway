 ...
 *
 * <p>WebFilter는 Spring WebFlux 기반 애플리케이션에서 모든 컨트롤러 요청 전/후에 실행됩니다. Gateway 라우트 설정과 독립적으로 작동하므로,
 * 컨트롤러로 향하는 모든 요청에 적용됩니다.
 */
@Component
public class WebLoggingFilter implements WebFilter, Ordered {

  private static final Logger logger =
      LoggerFactory.getLogger("com.linkedout.apigateway.filter.WebLoggingFilter");
  ...
  /** 본문 로깅을 건너뛸 Content-Type 목록 바이너리 데이터나 대용량 데이터는 로깅하지 않음 */
  ...
  /** Content-Type을 확인하여 본문 로깅 여부 결정 바이너리 데이터나 대용량 데이터는 로깅하지 않음 */
